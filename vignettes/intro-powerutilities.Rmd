...
```
Note that here the default, "asympotic", method for determining denominator degrees of freedom was used. If we wanted to use another method, it should be done when calculating the estimated marginal means:
...
Using the `merge` function will then replicate the whole treatment dictiorary for every row (i.e., block) in our design data frame:
