...
The basic idea is to encode information about sample size and  
expected treatments means (and differences) for the proposed experiment in a 
fake data set, to then specify a model encoding information about the design of 
the proposed experiment, and finally to "fit" the model with plug-in values for 
the random effects and dispersion parameters (based on pilot studies, previously
published research or estimated guesswork). The result is that the mixed modelling
software does all the requisite work of calculating standard errors upon which
